...
window.initZombieGame = function() {
    ...
    // Đổi con trỏ chuột thành tâm ngắm
    ...
    window.addEventListener('keydown', e => { 
        ...
        if(e.key==='r'||e.key==='R') Game.keys.r=true;
        if(e.key==='g'||e.key==='G') Game.keys.g=true;
    });
    window.addEventListener('keyup', e => { 
        ...
        if(e.key==='r'||e.key==='R') Game.keys.r=false;
        if(e.key==='g'||e.key==='G') Game.keys.g=false;
    });
...
function gameLoop() {
    ...
    for (let i = Game.grenades.length - 1; i >= 0; i--) {
        let g = Game.grenades[i]; g.update(); g.draw(Game.ctx);
        if (g.isDestroyed) Game.grenades.splice(i, 1);
    }
    ...
    for (let i = Game.zombies.length - 1; i >= 0; i--) {
        ...
        for (let j = Game.bullets.length - 1; j >= 0; j--) {
            ...
            if (Physics.checkCircleCollision(z, b) && z.hp > 0) {
                ...
                AudioSys.play('hit', 0.2); // Tiếng trúng đạn
    ...
    // --- XỬ LÝ ÁNH SÁNG & ĐÈN PIN ---
    Game.ctx.fillStyle = 'rgba(5, 5, 10, 0.94)'; 
    ...
    Game.ctx.fillStyle = ambientLight; Game.ctx.beginPath(); Game.ctx.arc(Game.localPlayer.x, Game.localPlayer.y, 120, 0, Math.PI*2); Game.ctx.fill();

    let aimAngle = Physics.getAngle(Game.localPlayer.x, Game.localPlayer.y, Game.worldMouse.x, Game.worldMouse.y);
    let beamLength = 450; let beamWidth = Math.PI / 3.5; 
    ...
    Game.ctx.fillStyle = beamLight; Game.ctx.fill();

    // Ánh sáng chớp từ vụ nổ bom
    if (Game.explosionLight && Game.explosionLight.life > 0) {
        let exLight = Game.ctx.createRadialGradient(Game.explosionLight.x, Game.explosionLight.y, 0, Game.explosionLight.x, Game.explosionLight.y, 400);
        exLight.addColorStop(0, `rgba(255, 255, 255, ${Game.explosionLight.life/10})`); exLight.addColorStop(1, 'rgba(255, 255, 255, 0)');
        Game.ctx.fillStyle = exLight; Game.ctx.beginPath(); Game.ctx.arc(Game.explosionLight.x, Game.explosionLight.y, 400, 0, Math.PI*2); Game.ctx.fill();
